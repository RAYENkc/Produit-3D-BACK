...
router.post('/signin',async (req,res)=>{
    ...
    try {
        ...
        let token = jwt.sign({id: client._id, name: client.nom, role: 'CLIENT'}, process.env.ACCESS_TOKEN_SECRET,{expiresIn:'12h'});
        res.header('x-access-token',token).json({ message : 'Login Success !!!', token : token, role: 'CLIENT' });
        }else{
