...
router.post("/add", [upload], async (req, res) => {
  try {
    ...
    if (results.error)
      return res
        .status(200)
        .json({ message: results.error.details[0].message });
...
router.get("", async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: "Error get All Products :" + error.message });
  }
});
...
// [auth,autoris,
router.put("/editimage/:id", upload, async (req, res) => {
  let id = req.params.id;
  let product = await Produit.findById(id);
  if (!product) return res.status(200).json({ message: "Product Not Exist" });
  if (req.file) {
